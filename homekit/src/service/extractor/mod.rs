...
pub struct TaggedValue<T>(pub T::Value)
where
    T: Tlv8Pack;

impl<T> IntoResponse for TaggedValue<T>
where
    T: endec::Tlv8Encode,
{
    fn into_response(self) -> Response {
        (
            ...
            T::bytes_iter(self.0).collect::<Bytes>(),
        )
            .into_response()
    }
}

impl<S, T> FromRequest<S> for TaggedValue<T>
where
    S: Send + Sync,
    T: endec::Tlv8Decode,
{
    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        ...
        T::from_iter(Iter {
            buf: &Bytes::from_request(req, state).await?,
        })
        .map(Self)
    }
}
...
mod tests {
    ...
    async fn parse_tuple_public_key_and_another_tag() {
        ...
        let m_data = vec![MethodCode::PairSetup as _];
        ...
        let parsed: TaggedValue<(PublicKey, Method)> = TaggedValue::from_request(request, &())
            .await
            .expect("Failed to parse tuple");
        assert_eq!(parsed.0, (pk_data, MethodCode::PairSetup));
    }
}
